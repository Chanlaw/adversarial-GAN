...
real_images = tf.placeholder(tf.float32, [None, MNIST_DIMENSION], name="real_images")
...
noise, gen_images = generator.build_generator()
d_output, d5 = discriminator.build_discriminator(gen_images, real_images, labelled_images)
...
with tf.name_scope('eval'):
    ...
    loss_d, loss_d_unl, loss_d_lbl, loss_d_fake  = discriminator.calculate_loss(logits_unl, logits_fake, logits_lbl, num_lbl, labels)
    loss_g = generator.calculate_loss(d5, num_unl, num_lbl)
...
with tf.name_scope('training'):
    d_train_op = discriminator.train(loss_d, FLAGS.max_gradient_norm, FLAGS.learning_rate)
    g_train_op = generator.train(loss_g, FLAGS.learning_rate)
...
with tf.name_scope('image_summaries'):
    gen_images_uint = tf.reshape(tf.cast((gen_images+FLAGS.epsilon)*255.0/(1+2*FLAGS.epsilon), tf.uint8), [batch_size, 28, 28, 1])
...
with sess.as_default():
    ...
    for epoch in range(FLAGS.num_epochs):
        x_unl = x_unl[np.random.permutation(x_unl.shape[0])] #Copy of x_train
        x_unl2 = x_unl2[np.random.permutation(x_unl2.shape[0])] #Copy of x_train
        x_lbl = []
        y_lbl = []
